...
export const createOrder = asyncHandler(async (req, res) => {
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        mode: 'payment',
        success_url: `${process.env.FRONTEND_PATH}/success/${newOrder?._id.toString()}`,
        cancel_url: `${process.env.FRONTEND_PATH}/cancel/${newOrder?._id.toString()}`,
...
export const handleStripeWebhook = asyncHandler(async (req, res) => {
    let singInSecret = "whsec_89a5aea11d5a4cb7622024406ec370d2f8357dd995f256938b9f47f882e641f1"
...
console.log("hello")
    ...
    switch (event.type) {
        case 'checkout.session.completed':
            ...
            console.log(
                `Order ${orderId} completed successfully. Invoice URL: ${invoiceUrl}`
            )
            break
...
export const updateOrderStatus = asyncHandler(async (req, res) => {
    ...
    switch (session.payment_status) {
        case 'paid':
            ...
            await order.save();
            res.status(200).json(
                new ApiResponse(200, order, 'Order status and invoice updated successfully.')
            );
            break;
        ...
        case 'canceled':
            throw new ApiError(400, 'Payment was canceled. Unable to process the order.');
